...
module.exports = class ContractService {
  ...
  static async confirmContract(planContract) {
    try {
      ...
      let plan = await Database('plan')
        .where({ id_plan: planContract.fk_plan })
        .first();

      expires.setMonth(date.getMonth() + plan.months_of_validity);
      let row = await Database('plan_contract')
        .where({ id_plan_contract: planContract.id_plan_contract })
        .update({
          confirm_date: date,
          expires_in: expires,
        });
  ...
  static async getContractListByUser(id_user) {
    try {
      let planItems = await Database('plan_contract').where(
        'fk_user',
        '=',
        id_user
      );
  ...
  static async getAllContractsByUser(id_user) {
    try {
      return await Database('plan_contract')
        .where({ fk_user: id_user })
        .innerJoin('plan', 'plan_contract.fk_plan', 'plan.id_plan');
    } catch (error) {
      throw new Error('ContractService.getAllContractsByUser: ' + error);
    }
