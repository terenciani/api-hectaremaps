      ...
      if (area == 'logo') {
        upload(req, res, async function (err) {
          if (err) {
            res
              .status(200)
              .send({ status: 500, message: 'Erro ao enviar a imagem!' });
          } else {
            ...
            res
              .status(200)
              .send({ status: 200, message: 'Imagem atualizada com sucesso!' });
          }
        });
    ...
    try {
      upload(req, res, async function (err) {
        if (err) {
          res
            .status(200)
            .send({ status: 500, message: 'Erro ao enviar o vídeo!' });
        } else {
          let siteData = await ConfigService.getData();
          ...
          res
            .status(200)
            .send({ status: 200, message: 'Vídeo atualizado com sucesso!' });
        }
      });
    } catch (error) {
